...
ID<- paste0(Sys.Date(),"_",run,"_ITS_" )
...
namesF<- sapply(strsplit(basename(fnFs), "_its"), `[`, 1)
namesR<- sapply(strsplit(basename(fnRs), "_its"), `[`, 1)
match(namesF,namesR)
...
REV<- "AACTTTYRRCAAYGGATCWCT" # R1
FWD<- "AGCCTCCGCTTATTGATATGCTTAART" # R2
...
track <- cbind(do.call('rbind',reads_passed), 
               data.frame(reads_denoised=no_reads,final_reads=rowSums(nochim.seqtab),chimera_sum= chimera_sum, chimeric_sv=chimera_svs))
