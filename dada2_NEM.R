...
seq_path = file.path(getwd(),"/",paste0(run),"/NEM/")
...
ID<- paste0(Sys.Date(),"_",run,"_NEM_" )
...
namesF<- sapply(strsplit(basename(fnFs), "_NEM"), `[`, 1)
namesR<- sapply(strsplit(basename(fnRs), "_NEM"), `[`, 1)
match(namesF,namesR)
